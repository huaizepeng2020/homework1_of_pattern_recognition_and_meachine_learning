...
class Dataset(Methodset):
    ...
    def importdata(self, **kwargs):
        if "filename" in kwargs:
            ...
            dataset = pd.read_csv(kwargs["url"], index_col=kwargs["index_col"], header=kwargs["header"], usecols=kwargs["usecol"])
            # dataset = pd.read_csv(kwargs["url"], index_col=kwargs["index_col"], usecols=kwargs["usecol"])
...
class TrainData(Dataset):
    ...
    def split_into_folds(self, n_folds):
        ...
        for i, (iteration, index_data) in enumerate(kf.split(self.data)):
