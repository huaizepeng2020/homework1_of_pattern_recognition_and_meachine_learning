    ...
    def get_classconditional_probability(self, x, dataset, h_result):
        ...
        for i in range(dataset.num):
            ...
            for index_name in dataset.data.keys():
                ...
                # XX=float(x[index_name-float(dataset.data.keys()[0])]-x_i)/float(index_h[index_name])
                XX=float(x[list(dataset.data.keys()).index(index_name)]-x_i)/float(index_h[index_name])
