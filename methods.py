...
class Methodset:
    ...
    def get_max_decision(self,a_dic):
        ...
        max_key=list(key_list)[list(value_list).index(max_value)]
        return max_key
    ...
    def get_sample_covariance_matrix(self, sample_data):
        ...
        # covMatrix = sample_data_1.cov().values(columns=None)
    ...
    def modify_covariance(self,cov):
        ...
        for i in range(len(a)):
            ...
            if index_i<len(a)-1:
                ...
                if 0.99 <= cc:
                    for j in range(index_i+1,len(a)):
                        a_modeified[j] = delta
                    break
        cov_modefied = np.dot(np.dot(np.linalg.inv(b), mat(diag(a_modeified))),b)
        return cov_modefied
